...
\title{Reinforced Data Selection Guarantees Continuous Training for Large Pre-trained Models}

%这个工作最好可以用到GPT的API做实验（需要可以finetune的那种API，或者我们本地搭建环境），实验步骤大致这样:
%1.开始训练（或者FT），然后中断这个训练
%2.继续训练，比较两种情况，一种是直接用新数据继续FT，一种是通过数据选择选取老的数据跟新数据一起FT
%3.在数据选择阶段，使用两种方法，一种是简单TDS，通过比率选择，按照熵的原则选取相近case（直到预设的比率达到为止）；一种是强化学习选择，参考我的19年的ACL
%4.比较各个结果，应该是强化学习》比率选择》不选择
%5.简单分析各个结果
